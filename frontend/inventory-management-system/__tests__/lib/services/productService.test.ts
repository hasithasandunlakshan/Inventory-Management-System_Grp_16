...
describe('productService', () => {
  ...
  it('getProductById returns data on 200', async () => {
    mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ id: '7' }) });
  ...
  it('getProductById throws 404 as not found', async () => {
    ...
    await expect(productService.getProductById('x')).rejects.toThrow('Product not found');
  });

  it('addProduct posts body and returns created', async () => {
    const body = { name: 'A', description: 'B', price: 1, stock: 1 } as any;
    mockFetch.mockResolvedValueOnce({ ok: true, json: async () => ({ id: 'new', ...body }) });
  ...
  it('deleteProduct calls DELETE and throws on failure', async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(expect.stringMatching(/\/1$/), expect.objectContaining({ method: 'DELETE' }));
  ...
  it('updateProduct calls PUT and returns updated', async () => {
    const product = { id: 'u1', name: 'X', description: 'Y', price: 1, stock: 1 } as any;
    mockFetch.mockResolvedValueOnce({ ok: true, json: async () => product });
    const res = await productService.updateProduct(product);
    expect(res).toEqual(product);
    expect(mockFetch).toHaveBeenCalledWith(expect.stringMatching(/\/u1$/), expect.objectContaining({ method: 'PUT' }));

    mockFetch.mockResolvedValueOnce({ ok: false });
    await expect(productService.updateProduct(product)).rejects.toThrow(/Failed to update product/);
  });
});

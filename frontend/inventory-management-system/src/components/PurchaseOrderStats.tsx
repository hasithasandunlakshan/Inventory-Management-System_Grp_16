...
export function PurchaseOrderStats({ refreshTrigger, onStatsLoaded, defaultFilters }: PurchaseOrderStatsProps) {
  ...
  const [supplierId, setSupplierId] = useState(defaultFilters?.supplierId?.toString() || '');
  ...
  }, [isAuthenticated, onStatsLoaded, searchTerm, status, supplierId, dateFrom, dateTo]);
  ...
  const getPendingOrders = () => {
    ...
    return (stats.byStatusCounts.PENDING || 0) + (stats.byStatusCounts.SENT || 0) + (stats.byStatusCounts.DRAFT || 0);
  };
  const getReceivedOrders = () => stats?.byStatusCounts?.RECEIVED || 0;
  
  const completionRate = getTotalOrders() > 0 

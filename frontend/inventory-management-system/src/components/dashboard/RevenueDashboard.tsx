...
import { formatCurrency, formatMonth, getCurrentMonth } from "@/utils/revenueUtils";
...
export default function RevenueDashboard() {
  ...
  const yearToDateRevenue = monthlyRevenue.reduce((sum, month) => sum + month.revenue, 0);
  const yearToDateOrders = monthlyRevenue.reduce((sum, month) => sum + month.count, 0);
  ...
  return (
    <Card className="col-span-full">
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle>Revenue Dashboard</CardTitle>
          <CardDescription>Complete revenue overview and analytics</CardDescription>
        </div>
        <Button onClick={fetchAllRevenueData} variant="outline" size="sm" disabled={loading}>
          Refresh
        </Button>
      </CardHeader>
      <CardContent>
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              <div className="text-2xl font-bold">
                {formatCurrency(todayRevenue?.revenue || 0, todayRevenue?.currency)}
              </div>
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              <div className="text-2xl font-bold">
                {formatCurrency(currentMonth?.revenue || 0, currentMonth?.currency)}
              </div>
              <div className="text-sm text-muted-foreground">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-1">
              ...
              <div className="text-sm text-muted-foreground">
                {stripeStats?.total_payments || 0} payments, {stripeStats?.total_refunds || 0} refunds
              </div>
            </div>
          </div>
        </div>
                ...
                <div key={monthData.month} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-medium">
                      {formatCurrency(monthData.revenue, monthData.currency)} ({monthData.count} orders)
                    </span>
                  </div>

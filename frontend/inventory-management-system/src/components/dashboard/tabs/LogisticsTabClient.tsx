...
interface Props {
  initialLogisticsData: LogisticsMetrics | null;
}

export default function LogisticsTabClient({ initialLogisticsData }: Props) {
  const [logisticsData, setLogisticsData] = useState<LogisticsMetrics | null>(
    initialLogisticsData
  );
  const [loading, setLoading] = useState(!initialLogisticsData);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    // If no initial data, fetch it client-side
    if (!initialLogisticsData) {
      loadLogisticsData();
    }
  }, [initialLogisticsData]);

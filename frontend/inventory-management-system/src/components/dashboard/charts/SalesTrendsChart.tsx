...
export default function SalesTrendsChart() {
  ...
  useEffect(() => {
    const fetchSalesData = async () => {
      ...
      try {
        ...
        const mockData = generateMockSalesData(startDate, endDate, filters.warehouse);
        setSalesData(mockData);
        
        // Calculate trend
        if (mockData.length > 1) {
          const recent = mockData.slice(-3).reduce((sum, item) => sum + item.revenue, 0);
          const previous = mockData.slice(0, -3).reduce((sum, item) => sum + item.revenue, 0) || 1;
  ...
  const generateMockSalesData = (startDate: Date, endDate: Date, warehouse: string): SalesData[] => {
    const data: SalesData[] = [];
    const currentDate = new Date(startDate);
    
    while (currentDate <= endDate) {
      const baseRevenue = warehouse === 'wh1' ? 800 : warehouse === 'wh2' ? 600 : 1400;
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-end gap-1 h-32">
              {salesData.slice(-10).map((item, index) => {
                const height = (item.revenue / Math.max(...salesData.map(d => d.revenue))) * 100;
                return (
          ...
          <div className="text-xs text-gray-500 flex justify-between">
            <span>Warehouse: {filters.warehouse === 'all' ? 'All' : filters.warehouse.toUpperCase()}</span>

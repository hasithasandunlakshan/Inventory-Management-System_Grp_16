...
const NotificationBell = () => {
  ...
  async function load() {
    try {
      ...
      const deduped = Array.from(latestByKey.values()).sort((x, y) => new Date(y.createdAt) - new Date(x.createdAt));
                  ...
                  <div className="flex items-start space-x-3">
                    <div className="flex-shrink-0">
                      <div className={`w-2 h-2 rounded-full mt-2 ${
                        !notification.isResolved ? 'bg-red-500' : 'bg-gray-300'
                      }`} />
                    </div>

                    <div className="flex-1 min-w-0">
                      <p className={`text-sm ${!notification.isResolved ? 'font-semibold text-gray-900' : 'text-gray-700'}`}>
                        {notification.message}
                        {notification._count > 1 && (
                          <span className="ml-2 text-xs text-gray-500">(+{notification._count - 1} more)</span>
                        )}
                      </p>
                      <p className="text-xs text-gray-500 mt-1">
                        {new Date(notification.createdAt).toLocaleString()} · {notification.alertType}
                      </p>
                    </div>
                    {!notification.isResolved && (
                      <button
                        ...
                        onClick={async () => {
                          try {
                            await stockAlertService.resolve(notification.alertId);
                            setNotifications(prev => prev.map(x => x.alertId === notification.alertId ? { ...x, isResolved: true } : x));

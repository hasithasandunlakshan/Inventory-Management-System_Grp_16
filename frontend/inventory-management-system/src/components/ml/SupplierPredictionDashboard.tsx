...
export default function SupplierPredictionDashboard({ className }: SupplierPredictionDashboardProps) {
  ...
  const loadData = async () => {
    try {
      ...
      const [supplierData, supplierIds] = await Promise.all([
        ...
        supplierService.getAllSuppliers().then(suppliers => suppliers.map(s => s.supplierId))
      ]);
  ...
  const averageScore = predictions.length > 0 
    ? predictions.reduce((sum, p) => sum + p.reliability_score, 0) / predictions.length 
    : 0;

  const highRiskCount = predictions.filter(p => p.reliability_score < 0.6).length;
  const mediumRiskCount = predictions.filter(p => p.reliability_score >= 0.6 && p.reliability_score < 0.8).length;
  const lowRiskCount = predictions.filter(p => p.reliability_score >= 0.8).length;
          ...
          <div className="space-y-6">
              ...
              <div>
                ...
                <div className="grid gap-3">
                  {predictions
                    ...
                    .map((prediction) => (
                      <div key={prediction.supplier_id} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-red-200">
                        <div>
                          <div className="font-medium">{getSupplierName(prediction.supplier_id)}</div>
                          <div className="text-sm text-gray-600">ID: {prediction.supplier_id}</div>
                        </div>
            ...
            {mediumRiskCount > 0 && (
              <div>
                ...
                <div className="grid gap-3">
                  {predictions
                    .filter(p => p.reliability_score >= 0.6 && p.reliability_score < 0.8)
                    .sort((a, b) => b.reliability_score - a.reliability_score)
                    .map((prediction) => (
                      <div key={prediction.supplier_id} className="flex items-center justify-between p-3 bg-yellow-50 rounded-lg border border-yellow-200">
                        <div>
                          <div className="font-medium">{getSupplierName(prediction.supplier_id)}</div>
                          <div className="text-sm text-gray-600">ID: {prediction.supplier_id}</div>
                        </div>
              ...
              <div>
                ...
                <div className="grid gap-3">
                  {predictions
                    ...
                    .sort((a, b) => b.reliability_score - a.reliability_score)
                    .map((prediction) => (
                      <div key={prediction.supplier_id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg border border-green-200">
                        <div>
                          <div className="font-medium">{getSupplierName(prediction.supplier_id)}</div>
                          <div className="text-sm text-gray-600">ID: {prediction.supplier_id}</div>
                        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">
                {predictions.length > 0 ? ((lowRiskCount / predictions.length) * 100).toFixed(1) : 0}%
              </div>
              <div className="text-sm text-gray-600">Low Risk %</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-red-600">
                {predictions.length > 0 ? ((highRiskCount / predictions.length) * 100).toFixed(1) : 0}%
              </div>
              <div className="text-sm text-gray-600">High Risk %</div>
            </div>
          </div>

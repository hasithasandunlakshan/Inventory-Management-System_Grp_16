...
import {
  SearchSyncService,
  SyncResult,
} from '@/lib/services/searchSyncService';
    ...
    <div className='space-y-6'>
      ...
      <div>
        ...
        <p className='text-gray-600 mt-2'>
          Intelligent search across products, suppliers, documents, and purchase
          orders using Azure AI Search
        </p>
          ...
          <div className='mt-4 flex items-center justify-between'>
            <div className='flex items-center space-x-4'>
              <div className='flex items-center space-x-2'>
                <div
                  className={`w-2 h-2 rounded-full ${backendServices.products ? 'bg-green-500' : 'bg-red-500'}`}
                ></div>
                <span className='text-sm text-gray-600'>Products Service</span>
              </div>
              <div className='flex items-center space-x-2'>
                <div
                  className={`w-2 h-2 rounded-full ${backendServices.suppliers ? 'bg-green-500' : 'bg-red-500'}`}
                ></div>
                <span className='text-sm text-gray-600'>Suppliers Service</span>
              </div>
              {!backendServices.overall && (
                <span className='text-sm text-red-600'>
                  ⚠️ Backend services not available
                </span>
              )}
            </div>
            <Button
      ...
      <Card>
        <CardHeader>
          ...
          <p className='text-sm text-gray-600'>
            Sync your existing database data to Azure Search for intelligent
            search capabilities
          </p>
        </CardHeader>
        <CardContent>
          ...
          {!backendServices?.overall && (
            <Alert variant='destructive'>
              <AlertCircle className='h-4 w-4' />
              <AlertDescription>
                Backend services are not available. Please start your inventory
                and supplier services to sync data.
              </AlertDescription>
            </Alert>
          )}
        </CardContent>

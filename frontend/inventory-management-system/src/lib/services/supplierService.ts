...
class SupplierService {
  ...
  constructor() {
    console.log('🏭 SupplierService initialized with direct URL:', this.baseUrl);
  }
  ...
  private safeExtractMonth(dateString: string | null | undefined): string | null {
  ...
  async getAllSuppliers(): Promise<SupplierDTO[]> {
    try {
      console.log('🏭 Fetching suppliers directly from supplier service:', `${this.baseUrl}/api/suppliers`);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch suppliers: ${response.status} ${response.statusText} - ${errorText}`);
      }
  ...
  async getSupplierById(id: number): Promise<SupplierDTO> {
    try {
      console.log('🏭 Fetching supplier directly from supplier service:', `${this.baseUrl}/api/suppliers/${id}`);
      ...
      if (!response.ok) {
        const errorText = await response.text();
        console.error('🏭 Supplier fetch error:', errorText);
        throw new Error(`Failed to fetch supplier: ${response.status} ${response.statusText} - ${errorText}`);
      }
      const data = await response.json();
  ...
  async getAllCategories(): Promise<SupplierCategoryDTO[]> {
    try {
      console.log('🏭 Fetching supplier categories directly from supplier service:', `${this.baseUrl}/api/supplier-categories`);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch supplier categories: ${response.status} ${response.statusText} - ${errorText}`);
      }
      const data = await response.json();
  ...
  async getAllPurchaseOrders(): Promise<PurchaseOrderSummaryDTO[]> {
    try {
      console.log('🏭 Fetching purchase orders directly from supplier service:', `${this.baseUrl}/api/purchase-orders`);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch purchase orders: ${response.status} ${response.statusText} - ${errorText}`);
      }
      const data = await response.json();
  ...
  async getPurchaseOrderStats(params?: {
    ...
    try {
      ...
      console.log('🏭 Fetching purchase order stats directly from supplier service:', url);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch purchase order stats: ${response.status} ${response.statusText} - ${errorText}`);
      }
      const data = await response.json();
  ...
  async getSupplierSpend(supplierId: number, from?: string, to?: string): Promise<SupplierSpendDTO> {
    try {
      const queryParams = new URLSearchParams();
      ...
      console.log('🏭 Fetching supplier spend directly from supplier service:', url);
      ...
      if (!response.ok) {
        ...
        throw new Error(`Failed to fetch supplier spend: ${response.status} ${response.statusText} - ${errorText}`);
      }
      const data = await response.json();
  ...
  async getSupplierAnalytics(dateFrom?: string, dateTo?: string): Promise<PurchaseOrderStats> {
    try {
      ...
      const supplierSpendMap = new Map<number, { name: string; totalSpend: number; orderCount: number }>();
      ...
      purchaseOrders.forEach(order => {
        try {
          ...
          console.error('🏭 Error processing order for supplier spend:', error, 'Order:', order);

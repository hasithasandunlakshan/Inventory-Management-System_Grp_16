...
public class UserDiscountController {
    ...
    public ResponseEntity<?> getActiveProductDiscounts(@RequestParam(required = false) List<Long> productIds,
        ...
        try {
            ...
            if (productIds != null && !productIds.isEmpty()) {
                ...
                List<Discount> productDiscounts = discountService.getActiveDiscountsByType(DiscountType.PRODUCT_DISCOUNT);
    ...
    public ResponseEntity<?> validateAndCalculateDiscount(@RequestBody ApplyDiscountRequest request) {
        try {
            ...
            boolean canUse = discountService.canUserUseDiscount(discount.getId(), request.getUserId(), request.getOrderAmount());
            if (!canUse) {
                return ResponseEntity.ok(DiscountCalculationResponse.failure("You are not eligible to use this discount"));
            }
            
            // For product discounts, check if any cart products are eligible
            if (discount.getType() == DiscountType.PRODUCT_DISCOUNT) {
                boolean hasEligibleProducts = checkProductEligibility(discount.getId(), request.getProductIds(), request.getProductBarcodes());
                if (!hasEligibleProducts) {
                    return ResponseEntity.ok(DiscountCalculationResponse.failure("No eligible products in cart for this discount"));
                }
            }
            ...
            return ResponseEntity.ok(DiscountCalculationResponse.failure("Error validating discount: " + e.getMessage()));
        }
    }
     ...
     * If orderId is null/empty, performs validation and returns discount calculation without recording usage
     */
    @PostMapping("/apply")
    ...
    public ResponseEntity<?> checkDiscountEligibility(@PathVariable Long discountId,
        ...
        try {
            ...
            if (canUse) {
                return ResponseEntity.ok(new DiscountEligibilityResponse(true, "You are eligible to use this discount"));
            } else {
                return ResponseEntity.ok(new DiscountEligibilityResponse(false, "You are not eligible to use this discount"));
            }
        } catch (Exception e) {
    ...
    private UserDiscountHistoryResponse convertToUserDiscountHistoryResponse(UserDiscount userDiscount) {
        return UserDiscountHistoryResponse.builder()
                ...
                .originalAmount(userDiscount.getOriginalAmount() != null ? userDiscount.getOriginalAmount().doubleValue() : null)
                .discountAmount(userDiscount.getDiscountAmount() != null ? userDiscount.getDiscountAmount().doubleValue() : null)
                ...
                .discountValue(userDiscount.getDiscount().getDiscountValue() != null ? userDiscount.getDiscount().getDiscountValue().doubleValue() : null)
    ...
    public static class UserSavingsResponse {
        ...
        public Long getTotalDiscountsUsed() { return totalDiscountsUsed; }
    }

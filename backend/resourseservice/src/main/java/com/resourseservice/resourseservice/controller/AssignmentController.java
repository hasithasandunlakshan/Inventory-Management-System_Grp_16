...
public class AssignmentController {
        ...
        @GetMapping("/minimal")
        public ResponseEntity<ApiResponse<List<MinimalAssignmentResponse>>> getAllAssignmentsMinimal() {
                try {
                        List<MinimalAssignmentResponse> assignments = assignmentService.getAllAssignmentsMinimal();
                        return ResponseEntity.ok(ApiResponse.success(assignments,
                                        "Successfully retrieved " + assignments.size()
                                                        + " assignments with minimal details"));

                } catch (Exception e) {
                        log.error("Failed to get all assignments minimal: {}", e.getMessage(), e);
                        return ResponseEntity.status(500)
                                        .body(ApiResponse.error("Failed to retrieve assignments", e.getMessage()));
                }
        }

        @GetMapping("/active/minimal")
        public ResponseEntity<ApiResponse<List<MinimalAssignmentResponse>>> getActiveAssignmentsMinimal() {
                try {
                        List<MinimalAssignmentResponse> assignments = assignmentService.getActiveAssignmentsMinimal();
                        return ResponseEntity.ok(ApiResponse.success(assignments,
                                        "Successfully retrieved " + assignments.size()
                                                        + " active assignments with minimal details"));

                } catch (Exception e) {
                        log.error("Failed to get active assignments minimal: {}", e.getMessage(), e);
                        return ResponseEntity.status(500)
                                        .body(ApiResponse.error("Failed to retrieve active assignments",
                                                        e.getMessage()));
                }
        }

        @GetMapping("/{assignmentId}/details")
        public ResponseEntity<ApiResponse<DetailedAssignmentResponse>> getAssignmentDetails(
                        @PathVariable Long assignmentId) {
                try {
                        return assignmentService.getAssignmentDetailsById(assignmentId)
                                        .map(assignment -> ResponseEntity.ok(
                                                        ApiResponse.success(assignment, "Assignment details found")))
                        ...
                        log.error("Failed to get assignment details by ID {}: {}", assignmentId, e.getMessage(), e);
                        return ResponseEntity.status(500)
                                        .body(ApiResponse.error("Failed to retrieve assignment details",
                                                        e.getMessage()));
                }
        }
        ...
        public ResponseEntity<ApiResponse<List<AssignmentResponse>>> getAssignmentsByDriverId(
                ...
                try {
                        ...
                        return ResponseEntity.ok(ApiResponse.success(responses,
                                        "Successfully retrieved " + responses.size() + " assignments for driver ID: "
                                                        + driverId));

                } catch (Exception e) {
                        log.error("Failed to get assignments by driver ID {}: {}", driverId, e.getMessage(), e);
                        return ResponseEntity.status(500)
                                        .body(ApiResponse.error("Failed to retrieve assignments by driver",
                                                        e.getMessage()));
                }
        }
        ...
        public ResponseEntity<ApiResponse<List<AssignmentResponse>>> getAssignmentsByVehicleId(
                ...
                try {
                        ...
                        return ResponseEntity.ok(ApiResponse.success(responses,
                                        "Successfully retrieved " + responses.size() + " assignments for vehicle ID: "
                                                        + vehicleId));

                } catch (Exception e) {
                        log.error("Failed to get assignments by vehicle ID {}: {}", vehicleId, e.getMessage(), e);
                        return ResponseEntity.status(500)
                                        .body(ApiResponse.error("Failed to retrieve assignments by vehicle",
                                                        e.getMessage()));
                }
        }

        @GetMapping("/driver/{driverId}/active")
        public ResponseEntity<ApiResponse<AssignmentResponse>> getActiveAssignmentByDriverId(
                        @PathVariable Long driverId) {
                try {
                        return assignmentService.getActiveAssignmentByDriverId(driverId)
                                        .map(assignment -> ResponseEntity.ok(ApiResponse.success(
                                                        AssignmentResponse.fromEntity(assignment),
                                                        "Active assignment found for driver")))
                                        .orElse(ResponseEntity.status(404)
                                                        .body(ApiResponse.error("No active assignment",
                                                                        "No active assignment found for driver ID: "
                                                                                        + driverId)));

                } catch (Exception e) {
                        log.error("Failed to get active assignment by driver ID {}: {}", driverId, e.getMessage(), e);
                        return ResponseEntity.status(500)
                                        .body(ApiResponse.error("Failed to retrieve active assignment",
                                                        e.getMessage()));
                }
        }
        ...
        public ResponseEntity<ApiResponse<AssignmentResponse>> getActiveAssignmentByVehicleId(
                ...
                try {
                        return assignmentService.getActiveAssignmentByVehicleId(vehicleId)
                                        .map(assignment -> ResponseEntity.ok(ApiResponse.success(
                                                        AssignmentResponse.fromEntity(assignment),
                                                        "Active assignment found for vehicle")))
                                        .orElse(ResponseEntity.status(404)
                                                        .body(ApiResponse.error("No active assignment",
                                                                        "No active assignment found for vehicle ID: "
                                                                                        + vehicleId)));
        ...
        public ResponseEntity<ApiResponse<AssignmentResponse>> unassignDriverFromVehicle(
                ...
                try {
                        ...
                        Assignment unassignedAssignment = assignmentService.unassignDriverFromVehicle(assignmentId,
                                        unassignedBy,
                                        notes);
        ...
        public ResponseEntity<ApiResponse<AssignmentResponse>> unassignDriverFromVehicleByDriverId(
                ...
                try {
                        ...
                        Assignment unassignedAssignment = assignmentService.unassignDriverFromVehicleByDriverId(
                                        driverId,
                                        unassignedBy, notes);
                        AssignmentResponse response = AssignmentResponse.fromEntity(unassignedAssignment);
        ...
        public ResponseEntity<ApiResponse<AssignmentResponse>> unassignDriverFromVehicleByVehicleId(
                ...
                try {
                        ...
                        Assignment unassignedAssignment = assignmentService.unassignDriverFromVehicleByVehicleId(
                                        vehicleId,
                                        unassignedBy, notes);
                        AssignmentResponse response = AssignmentResponse.fromEntity(unassignedAssignment);

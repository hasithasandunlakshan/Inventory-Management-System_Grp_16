...
public class DriverController {
    ...
    @GetMapping("/available-with-vehicles")
    public ResponseEntity<ApiResponse<List<DriverWithVehicleResponse>>> getAvailableDriversWithVehicles() {
        try {
            List<DriverWithVehicleResponse> drivers = driverProfileService.getAvailableDriversWithVehicles();

            return ResponseEntity.ok(ApiResponse.success(drivers,
                    "Successfully retrieved " + drivers.size() + " available drivers with vehicles"));

        } catch (Exception e) {
            log.error("Failed to get available drivers with vehicles: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(ApiResponse.error("Failed to retrieve drivers with vehicles", e.getMessage()));
        }
    }
}
